...
public:
    constexpr RTCDateTime(const uint16_t year, const uint8_t month, const uint8_t day, const uint8_t hour = 0u,
                          const uint8_t min = 0u, const uint8_t sec = 0u) : yOff(static_cast<uint8_t>(year >= 2000u ? year - 2000u : year)),
                                                                            m(month), d(day),
    ...
    RTCDateTime(const char *date = __DATE__, const char *time = __TIME__)
    {
        ...
        const uint16_t year = (date[7] - '0') * 1000u +
                              ...
                              (date[9] - '0') * 10u +
                              (date[10] - '0');
        yOff = static_cast<uint8_t>(year >= 2000u ? year - 2000u : year);
    ...
    {
        return static_cast<uint16_t>(yOff) + 2000u;
    }
